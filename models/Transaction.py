...
class Transaction(BaseModel):
    ...
    def __str__(self):
        ...
        return "[%(transaction_id)s] | %(timestamp)s | (%(creditor)s)>-(%(amount)s %(currency)s)->(%(debtor)s) | %(original_amount)s %(original_currency)s -- %(description)s" % {
            "transaction_id": self.transaction_id,
